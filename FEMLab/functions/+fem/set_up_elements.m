...
[U, I] = slv.setup_known_U(elems, n_tri, nodes_prop, regions_c);
...
for i_el = n_lines+1:n_elements
    ...
    [elements(i_el).K, elements(i_el).R, elements(i_el).U, ...
        elements(i_el).n] = ...
        slv.calc_element_RK(U(i_tri,:)', fun_K, fun_R, f_K, f_R, nodes, ...
        nodes_prop, elements(i_el), ...
        regions_c, element_r(i_el), tri_area(i_tri),abc,...
        tri_x(i_tri,:), tri_y(i_tri,:), params(i_el), is_source(i_el), I(i_tri,:));
end
