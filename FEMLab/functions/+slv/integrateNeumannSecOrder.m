function I = integrateNeumannSecOrder(A, abc, f, fun, x, y, side)
...
switch side
    case 1
        tfun = @(iPt,x,y) slv.neumannIntegrationMapping(iPt, y, gData);
        ds = @neumannIntegrationFunDerivatives(side, x, y, iPt, gData)
    case 2
        tfun = @(iPt,x,y,gData) slv.neumannIntegrationMapping(iPt, x, gData);
        xt = -x(1)+x(3);
        yt = -y(1)+y(3);
        ds = sqrt(xt^2+yt^2);
    case 3
